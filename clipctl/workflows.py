...
def normalize_wan_length(seconds: float, fps: int) -> int:
    """Return the nearest valid Wan frame count in the 4n+1 sequence."""
...
def build_wan22_i2v_prompt(
    *,
    ...
    shift: float = 8.0,
    seed: int | None = None,
    filename_prefix: str = "clipctl/scene",
) -> dict[str, Any]:
    if width % 16 or height % 16:
        raise UserError(
            "Wan video genişlik ve yükseklik değerleri 16'nın katı olmalı."
        )
    if width > 960 or height > 544:
        raise UserError(
            "8 GB VRAM profili için çözünürlük fazla yüksek. "
            "Önerilen: 832x480; kurtarma: 704x400."
        )
    ...
    return {
        ...
        "4": {
            ...
            "inputs": {"text": positive, "clip": _ref("2")},
        },
        "5": {
            "class_type": "CLIPTextEncode",
            "inputs": {"text": negative, "clip": _ref("2")},
        },
        "6": {
        ...
        "8": {
            ...
            "inputs": {"model": _ref("1"), "shift": shift},
        },
        "9": {
            "class_type": "KSampler",
            "inputs": {
        ...
        "10": {
            ...
            "inputs": {"samples": _ref("9"), "vae": _ref("3")},
        },
        "11": {
            "class_type": "CreateVideo",
            "inputs": {"images": _ref("10"), "fps": fps},
        },
        "12": {
    ...
    for node_id, node in adapted.items():
        ...
        for name in required:
            if name not in inputs:
                errors.append(
                    f"Node {node_id}/{class_type}: zorunlu input eksik: {name}"
                )
    if missing_classes:
        raise UserError(
            "ComfyUI içinde gerekli node'lar bulunamadı: "
            + ", ".join(sorted(set(missing_classes)))
            + "\nComfyUI'ı güncelle ve server.log içindeki import "
            "hatalarını kontrol et."
        )
    if errors:
        raise UserError(
            "Workflow kurulu ComfyUI sürümüyle uyuşmuyor:\n- "
            + "\n- ".join(errors)
        )
    return adapted
